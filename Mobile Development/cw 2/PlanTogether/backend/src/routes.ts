...
import HandlerApi from "./api/handler";
import ListTaskByDateRangeApi from "./api/listTaskByDateRange";
...
export function addRoutesToExpressInstance(app: Express): void {
  ...
  app.get("/check-handler/:handler", handleError(HandlerApi));
  app.get("/get-user-task/:dateRange", handleError(ListTaskByDateRangeApi));
  app.get("/list-friends", handleError(ListFriend));
